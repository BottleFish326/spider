...
from a1_mujoco_env_new import A1MujocoEnv
...
def test(args):
    ...
    model = PPO.load(path=model_path, env=env, verbose=1, device="cpu")
...
def test_real(args):
    ...
    env = A1RealEnv()
    # action = env.robot.ReceiveObservation()
    # print(action)
    #env.robot.stand_up(standup_time=1.5, reset_time=5)
    # for i in range(200):
    # # action = np.array([-0.0235997, 1.15558109, -2.71346021 , -0.11702925, 1.12541091, -2.72294235, -0.09397739, 1.09907198, -2.68661547, -0.6486547, 1.12490523, -2.69576049])
    #     print(action)
    #     # action[0] += 0.01
    #     # time.sleep(1.0 / 50)
    #     # # print(action)
    ...
    model = PPO.load(path=model_path, env=env_mujoco, verbose=1)
