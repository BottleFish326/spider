...
class A1MujocoEnv(MujocoEnv):
    ...
    def leg_lift_frequency_reward(self):
        # 定义一个阈值，用于判断是否抬腿（根据实际情况调整）
        lift_threshold = 0.1
        feet_site_ids = [self._feet_site_name_to_id[f] for f in ["FR", "FL", "RR", "RL"]]
        lift_count = 0
        prev_feet_heights = np.zeros(4)
        for site_id in feet_site_ids:
            foot_pos = self.data.site_xpos[site_id]
            current_height = foot_pos[2]
            if current_height - prev_feet_heights[site_id] > lift_threshold:
                lift_count += 1
            prev_feet_heights[site_id] = current_height
        frequency = lift_count / self._step if self._step > 0 else 0
        return frequency * self.reward_weights.get("leg_lift_frequency", 1.0)
